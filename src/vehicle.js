...
const vehicle = {
    ...
    prototype: {
        constructor(guidingPoint,drawbar,u){
            this._A = guidingPoint; // reference to previous point
            ...
            this._chassi = { b: 178.5, l: 427, img: "./honda.png" }
        ...
        driveForward(){ // Simulation der Vorwärtsfahrt oder entlang linearer Traktrix
            ...
            this._history.push({A: this.A.cpy, B: this.B.cpy, phi: this._phi});
            while(path > 0){
                path -= ds;
                this._history.push({A: this.A.cpy, B: this.B.cpy, phi:this._phi});
...
const world = {
    ...
    prototype: {
        constructor(ctx,{x,y}){
            this._origin = {x:x,y:y,cartesian:true,scl: () => this.scale};
            this._ctx = ctx;
            this._interactor = canvasInteractor.create(this._ctx, {x,y,cartesian,scl} = this._origin );
            ...
            //this._g = g2().clr().img({uri:roundaboutUrl,b:2000,h:1500,x: -500,y:-500}).view(this._interactor.view)/* .lin({x1:0,y1:1000,x2:0,y2:-1000,lw:2}).lin({x1:-500,y1:0,x2:1000,y2:0,lw:2}) */;
        ...
        init(){
            this._g
                    .img({uri: () => this._showChassi ? this.vehicle._chassi.img : undefined, xoff: -170, yoff: -180, x: () => this.vehicle.B.x, y: () => this.vehicle.B.y, scl:0.65, w: () => this.vehicle.e.w})
                    .ply({pts: () => this.tractrix(), lw: 5, ls: 'Crimson'})
                    .vec({p1:{x:0,y:0}, p2: {x:1000,y:0}, lw:2, label:{str:'x', loc:0.95}})
                    .vec({p1:{x:0,y:0}, p2: {x:0,y:300},lw:2, label:{str:'y', off:-5,loc:0.9}})
                   ...
                   .ply({pts: () => {
                    ...
                    .use({grp: () => {
                        ...
                        let g = g2();
                        for(let nodes of this._history.flat()) g.ply({pts: () => nodes, fs:'red'});
                        return g;
                    }}) */
                    ...
                    .hdl( {p: () => this.vehicle.handle, label:'Z'})
        ...
        createTrailer(drawbar){
            const previous = this.trailers[this.trailers.length - 1];
            const next = vehicle.create(previous.B, drawbar, () => previous.e);
        ...
        simulateStep(){
            ...
            this._g.ply({pts: [this.vehicle.A.cpy, this.vehicle.B.cpy], lw: 2.5, ls:'#003d61' }).nod({p: this.vehicle.A.cpy}).nod({p:this.vehicle.B.cpy})
        ...
        onpan(e){ 
            //this._path.push({x:e.xusr, y:e.yusr})
